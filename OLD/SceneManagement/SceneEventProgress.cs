...
using AsyncOperation = Netcode.io.OLD.UnityEngine.AsyncOperation;
...
namespace Netcode.io.OLD.SceneManagement
{
    ...
    internal class SceneEventProgress
    {
        ...
        {
            if (!ClientsProcessingSceneEvent.ContainsKey(clientId)) return;
        ...
        /// </summary>
        private IEnumerator TimeOutSceneEventProgress()
        {
        ...
        private bool IsNetworkSessionActive()
            => m_NetworkManager != null && m_NetworkManager.IsListening && !m_NetworkManager.ShutdownInProgress;

        /// <summary>
        /// Will try to finish the current scene event in progress as long as
        /// all conditions are met.
        /// </summary>
        private void TryFinishingSceneEventProgress()
        {
            if (!HasFinished() && !HasTimedOut()) return;
