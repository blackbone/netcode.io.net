...
namespace Netcode.io.OLD.SceneManagement
{
    ...
    internal class SceneEventData : IDisposable
    {
...
#else
            var inSceneNetworkObjects = m_NetworkManager.FindObjectsOfType<NetworkObject>(includeInactive: true).Where((c) => c.NetworkManager == m_NetworkManager);
        ...
        private void ReadClientReSynchronizationData(FastBufferReader reader)
        {
            ...
            if (networkObjectsToRemove.Length <= 0) return;
        ...
        private void DeserializeDespawnedInScenePlacedNetworkObjects()
        {
            ...
            for (int i = 0; i < despawnedObjectsCount; i++)
            {
                ...
                if (!sceneCache.ContainsKey(networkSceneHandle))
                {
                    ...
                    {
                        if (m_NetworkManager.SceneManager.ScenesLoaded.TryGetValue(localSceneHandle, out _))
                        {
                            // Find all active and non-active in-scene placed NetworkObjects
                            var inSceneNetworkObjects = m_NetworkManager.FindObjectsOfType<NetworkObject>(includeInactive: true).Where((c) =>
                            c.GetSceneOriginHandle() == localSceneHandle && c.IsSceneObject != false).ToList();

                            foreach (var inSceneObject in inSceneNetworkObjects)
                                sceneRelativeNetworkObjects.TryAdd(inSceneObject.GlobalObjectIdHash, inSceneObject);
