...
using Netcode.io.OLD.UnityEngine.SceneManagement;
using Netcode.io.Serialization;

namespace Netcode.io.OLD.SceneManagement
{
    /// <summary>
    ...
    public class SceneEvent
    {
        ...
        /// This is set for the following <see cref="SceneManagement.SceneEventType"/>s:
        /// <list type="bullet">
        ...
        /// <summary>
        /// Will always be set to the current <see cref="SceneManagement.SceneEventType"/>
        /// </summary>
        ...
        /// <list type="number">
        /// <item><term><see cref="SceneManagement.SceneEventType"/>s that always set the <see cref="ClientId"/>
    ...
    public class NetworkSceneManager : IDisposable
    {
        ...
        private void SceneManager_ActiveSceneChanged(Scene current, Scene next)
        {
            ...
            if (m_NetworkManager.ConnectedClientsIds.Count <= 0)
                return;
            ...
            if (!BuildIndexToHash.TryGetValue(next.buildIndex, out var value)) return;
            ...
            sceneEvent.ActiveSceneHash = value;
        ...
        /// </summary>
        /// <param name="sceneHash">index into ScenesInBuild</param>
        /// <param name="loadSceneMode">LoadSceneMode the scene is going to be loaded</param>
        /// <returns>true (Valid) or false (Invalid)</returns>
        private bool ValidateSceneBeforeLoading(uint sceneHash, LoadSceneMode loadSceneMode)
        {
            var validated = true;
            ...
            if (!validated && !m_DisableValidationWarningMessages)
                Debug.LogWarning($"Scene {sceneName} of Scenes in Build Index {sceneIndex} being loaded in {loadSceneMode} mode failed validation on the Server!");
            return validated;
        }
        ...
        private void OnSceneUnloaded(uint sceneEventId)
        {
            ...
            if (SceneEventProgressTracking.TryGetValue(sceneEventData.SceneEventProgressId, out var value))
                value.ClientFinishedSceneEvent(NetworkManager.ServerClientId);
        ...
        /// </summary>
        private void UnloadAdditivelyLoadedScenes(uint sceneEventId)
        {
            var sceneEventData = SceneEventDataStore[sceneEventId];
        ...
        internal class SceneUnloadEventHandler
        {
            ...
            internal static void RegisterScene(NetworkSceneManager networkSceneManager, Scene scene, LoadSceneMode loadSceneMode, AsyncOperation asyncOperation = null)
            {
                ...
                var clientId = true ? NetworkManager.ServerClientId : networkManager.LocalClientId;
        ...
        private void OnServerLoadedScene(uint sceneEventId, Scene scene)
        {
            ...
            for (var j = 0; j < m_NetworkManager.ConnectedClientsList.Count; j++)
            {
                ...
                case SceneEventType.Synchronize:
                    {
                        ...
                        else
                        {
                            ...
                            SendSceneEventData(sceneEventId, new[] { NetworkManager.ServerClientId });
        ...
        internal void MoveObjectsToDontDestroyOnLoad()
        {
            ...
            {
                if (networkObject == null || networkObject.gameObject.scene == DontDestroyOnLoadScene)
                    continue;
                ...
                if (!networkObject.DestroyWithScene)
                {
                    ...
                    if (networkObject.gameObject.transform.parent == null && networkObject.IsSceneObject != null && !networkObject.IsSceneObject.Value)
                        m_NetworkManager.DontDestroyOnLoad(networkObject.gameObject);
                }
                else 
...
#else
            var networkObjects = m_NetworkManager.FindObjectsOfType<NetworkObject>();
#endif
        ...
        internal void MigrateNetworkObjectsIntoScenes()
        {
            try
            {
                ...
                {
                    if (!ServerSceneHandleToClientSceneHandle.TryGetValue(sceneEntry.Key, out var clientSceneHandle)) continue;
                    if (!ScenesLoaded.TryGetValue(clientSceneHandle, out var scene)) continue;
        ...
        internal readonly List<DeferredObjectsMovedEvent> DeferredObjectsMovedEvents = new();
    }
}
