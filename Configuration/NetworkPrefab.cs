...
namespace Netcode.io.Configuration
{
    /// <summary>
    ...
    public class NetworkPrefab
    {
        ...
        public bool Validate(int index = -1)
        {
            ...
            if (Override == NetworkPrefabOverride.None)
            {
                if (Prefab == null)
                {
                    this.LogWarning($"{nameof(NetworkPrefab)} cannot be null ({nameof(NetworkPrefab)} at index: {index})");
                    return false;
                }
                ...
                if (networkObject == null)
                {
                    ...
                    {
                        this.LogWarning($"{NetworkManager.PrefabDebugHelper(this)} is missing " +
                ...
                case NetworkPrefabOverride.Hash:
                    {
                        if (SourceHashToOverride == 0)
                        {
                            if (NetworkLog.CurrentLogLevel <= LogLevel.Error)
                            {
                                this.LogWarning($"{nameof(NetworkPrefab)} {nameof(SourceHashToOverride)} is zero " +
                ...
                case NetworkPrefabOverride.Prefab:
                    {
                        if (SourcePrefabToOverride == null)
                        {
                            ...
                            {
                                this.LogWarning($"{nameof(NetworkPrefab)} {nameof(SourcePrefabToOverride)} is null (entry will be ignored).");
                                return false;
                            }
                        }
                        ...
                        if (!SourcePrefabToOverride.TryGetComponent(out networkObject))
                        {
                            if (NetworkLog.CurrentLogLevel <= LogLevel.Error)
                            {
                                this.LogWarning($"{nameof(NetworkPrefab)} ({SourcePrefabToOverride.name}) " +
            ...
            if (OverridingTargetPrefab == null)
            {
                if (NetworkLog.CurrentLogLevel <= LogLevel.Error)
                {
                    this.LogWarning($"{nameof(NetworkPrefab)} {nameof(OverridingTargetPrefab)} is null!");
                }
                switch (Override)
                {
                    case NetworkPrefabOverride.Hash:
                        {
                            this.LogWarning($"{nameof(NetworkPrefab)} override entry {SourceHashToOverride} will be removed and ignored.");
                            break;
                        }
                    case NetworkPrefabOverride.Prefab:
                        {
                            this.LogWarning($"{nameof(NetworkPrefab)} override entry ({SourcePrefabToOverride.name}) will be removed and ignored.");
                            break;
                        }
                }
                return false;
