...
{
    internal class DeferredMessageManager
    {
        ...
        {
            public float Expiry;
            public List<TriggerData> TriggerData;
        }

        protected readonly Dictionary<TriggerType, Dictionary<ulong, TriggerInfo>> m_Triggers = new();

        private readonly OLD.NetworkManager m_NetworkManager;

        internal DeferredMessageManager(OLD.NetworkManager networkManager)
        {
        ...
        /// </summary>
        public virtual unsafe void DeferMessage(TriggerType trigger, ulong key, FastBufferReader reader, ref NetworkContext context)
        {
        ...
        protected virtual void PurgeTrigger(TriggerType triggerType, ulong key, TriggerInfo triggerInfo)
        {
            if (NetworkLog.CurrentLogLevel <= LogLevel.Normal)
            {
                this.LogWarning($"Deferred messages were received for a trigger of type {triggerType} with key {key}, but that trigger was not received within within {m_NetworkManager.NetworkConfig.SpawnTimeout} second(s).");
            }
            ...
            triggerInfo.TriggerData.Clear();
        }

        public virtual void ProcessTriggers(TriggerType trigger, ulong key)
        {
            if (m_Triggers.TryGetValue(trigger, out var triggers))
            {
