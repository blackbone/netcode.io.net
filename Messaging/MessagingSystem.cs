...
namespace Netcode.io.Messaging
{
    ...
    internal class MessagingSystem : IDisposable
    {
        ...
        private List<ReceiveQueueItem> m_IncomingMessageQueue = new List<ReceiveQueueItem>(16);
        ...
        private Dictionary<ulong, List<SendQueueItem>> m_SendQueues = new Dictionary<ulong, List<SendQueueItem>>();
        ...
        public unsafe void Dispose()
        {
            ...
            for (var queueIndex = 0; queueIndex < m_IncomingMessageQueue.Count; ++queueIndex)
            {
                // Avoid copies...
                var item = m_IncomingMessageQueue.ElementAt(queueIndex);
                item.Reader.Dispose();
            }
        ...
        internal void SetServerMessageOrder(Span<uint> messagesInIdOrder)
        {
        ...
        public void HandleMessage(in MessageHeader header, FastBufferReader reader, ulong senderId, float timestamp, int serializedHeaderSize)
        {
            using (reader)
            {
                ...
                {
                    this.LogWarning($"Received a message with invalid message type value {header.MessageType}");
                    return;
                }
                ...
                if (handler == null)
                {
                    this.LogException(new HandlerNotRegisteredException(header.MessageType.ToString()));
                }
                else
                {
        ...
        {
            for (var index = 0; index < m_IncomingMessageQueue.Count; ++index)
            {
                // Avoid copies...
                var item = m_IncomingMessageQueue.ElementAt(index);
        ...
        internal void ClientConnected(ulong clientId)
        {
            ...
            m_SendQueues[clientId] = new List<SendQueueItem>();
        }
        ...
        internal int GetMessageVersion(Type type, ulong clientId, bool forReceive = false)
        {
            if (!m_PerClientMessageVersions.TryGetValue(clientId, out var versionMap))
            {
                if (forReceive)
                {
                    this.LogWarning($"Trying to receive {type.Name} from client {clientId} which is not in a connected state.");

                }
                else
                {
                    this.LogWarning($"Trying to send {type.Name} to client {clientId} which is not in a connected state.");
                }
                return -1;
            }
            ...
            where TClientIdListType : IReadOnlyList<ulong>
        {
            ...
            var sentMessageVersions = new HashSet<int>(clientIds.Count);
            ...
            where TMessageType : INetworkMessage
        {
            ...
            for (var i = 0; i < clientIds.Count; ++i)
            {
                ...
                {
                    var lastQueueItem = sendQueueItem.ElementAt(sendQueueItem.Count - 1);
                        ...
                        < tmpSerializer.Length + headerSerializer.Length)
                    {
                        sendQueueItem.Add(new SendQueueItem(delivery, NON_FRAGMENTED_MESSAGE_MAX_SIZE, Allocator.TempJob,
                            maxSize));
                        sendQueueItem.ElementAt(sendQueueItem.Count - 1).Writer.Seek(sizeof(BatchHeader));
                    }
                }

                var writeQueueItem = sendQueueItem.ElementAt(sendQueueItem.Count - 1);
        ...
        internal unsafe int SendMessage<T>(ref T message, NetworkDelivery delivery, in ulong[] clientIds)
            where T : INetworkMessage
        {
        ...
        internal unsafe void ProcessSendQueues()
        {
            foreach (var kvp in m_SendQueues)
            {
                ...
                {
                    var queueItem = sendQueueItem.ElementAt(i);
