namespace NetcodeIO.NET.Utils.Crypto
{
    ...
    public class Salsa20Engine : IStreamCipher
    {
        ...
        private readonly byte[] keyStream = new byte[StateSize * 4]; // expanded state, 64 bytes
        ...
        protected Salsa20Engine(int rounds)
        {
            if (rounds <= 0 || (rounds & 1) != 0) throw new ArgumentException("'rounds' must be a positive, even number");
        ...
        public virtual string AlgorithmName
        {
            ...
            {
                var name = "Salsa20";
                if (Rounds != DefaultRounds) name += "/" + Rounds;
                return name;
            }
        }
        ...
        public virtual byte ReturnByte(
            byte input)
        {
            if (LimitExceeded()) throw new Exception("2^70 byte limit per IV; Change IV");
        ...
        private static void SalsaCore(int rounds, IReadOnlyList<uint> input, IList<uint> x)
        {
            if (input.Count != 16)
        ...
        {
            if (++cW0 != 0) return false;
            if (++cW1 != 0) return false;
